...
It doesn't appear to cover the whole year. Is there another section to the table?
...
Now that we know there is more than one section, let's return the second item from the ResultSet.

```{python}
#| label: second-section

table_df2 = pd.read_html(str(tables))[0] # return the second item from the ResultSet

table_df2 
```

This is illustrative of the fact that you need to inspect the HTML of the page that you are querying to ensure that you are getting everything that you need. You also need to be aware of any changes that get made to the structure of the web page, lest your Python code fail to return everything that you are expecting.
...
Using the `StringIO()` class from the `io` library, create an in-memory stream of the data that can be operated on like a file, without having first saved it down as one. The .csv data is treated like a long string where fields are separated by delimiters (commas by default) and rows are separated by newline characters (typically `\n`). The `.read_csv()` method in Pandas converts this string into a DataFrame.
...
These are particularly important if you are making requests to free, public APIs, such as those provided by the government and NHS.
...
They may not be stated explicitly, but you should assume that these limits exists and throttle your calls appropriately.

#### Use caching

If the API isn't subject to frequent changes, cache results locally so that you do not need to make repeated requests for the same data.

#### Avoid polling too aggressively

- Make requests at a reasonable rate: do you really need to check for updates every second?. Link about what "polling" is : (https://medium.com/@sankalpa115/what-is-polling-b1ff70e87001)
- Make use of webhooks: these allow for automatic communication between systems, eliminating the need for one system to constantly check another for updates. Data is pushed automatically whenever an event occurs (https://www.geeksforgeeks.org/blogs/what-is-a-webhook-and-how-to-use-it/)
...
Some APIs require users to provide a `User-Agent` or use an API key so that the provider can get in contact if there is a problem
...
We have created a walkthrough of using the `fingertips_py` package [here](/intermediate-skills-sessions/18-webscraping-apis/fingertips_py.html).