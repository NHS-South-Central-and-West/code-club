...
- Sex (Female) - Coefficient = `{python} f"{model_sm.params['Sex']:.2f}"`. Being female increases log-odds of survival. On the probability scale, women had much higher survival rates.
- Age - Coefficient = `{python} f"{model_sm.params['Age']:.3f}"`. Younger passengers had slightly higher survival rates.
- Pclass - Coefficient = `{python} f"{model_sm.params['Pclass']:.2f}"`. Higher class numbers (lower class) decrease survival probability.
- Fare - Coefficient = `{python} f"{model_sm.params['Fare']:.3f}"`. Higher fares (correlated with class) increase survival.
