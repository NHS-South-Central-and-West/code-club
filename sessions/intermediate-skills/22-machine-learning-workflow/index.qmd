...
Every algorithm will have settings (hyperparameters) that aren't learned from data (e.g. max_depth, n_estimators). These need to be set before training the model. More complex algorithms, like random forests and gradient boosting models, have many hyperparameters, and their values can have a significant impact on model performance. The best approach is to tune the hyperparameters by testing multiple values and finding the combination of hyperparameter values that produce the best model performance. There are lots of different methods for hyperparameter optimisation, but the most common are Grid Search, Randomised Search, and Bayesian Optimisation[^Bayesian].

Grid Search (`GridSearchCV()` in scikit-learn) searches over a specified grid of values, evaluating each combination using cross validation. Because it uses cross validation internally, the selection process does not touch the held-out test set. This reduces the risk of the model overfitting.
...
Grid Search is exhaustive, but it is computationally expensive, and as the hyperparameter values Grid Search has to test increases, the time it will take increases exponentially. The more efficient approach is to use Randomised Search.

Randomised Search (`RandomizedSearchCV()` in scikit-learn) samples $n$ combinations (the `n_iter` argument in `RandomizedSearchCV()`) at random from the parameter distributions specified by the user.

```{python}
#| label: randomised-search
...
This is much faster than the exhaustive approach, and generally you will find that the randomised search will produce comparable results. 

However, nowadays these two approaches are better served as tools for learning how hyperparameter tuning works, because Bayesian Optimisation has replaced them as the industry standard, with [Optuna](https://optuna.org/) being the most popular library for this more sophisticated approach to tuning. Bayesian Optimisation finds the best hyperparameter values by sampling from the range or list provided by the user and using the results to identify the values to test next, sequentially learning from each trial. This is a more efficient approach and reduces time wasted searching any parameter space that is not producing good results.

Bayesian Optimisation and Optuna is a step too far for this session. The goal here is just to understand how hyperparameter tuning works generally, and why it is valuable in a machine learning workflow. But Optuna provides plenty of [examples](https://github.com/optuna/optuna-examples/) for using Optuna with common machine learning libraries, including [scikit-learn](https://github.com/optuna/optuna-examples/tree/main/sklearn). For more information about hyperparameter tuning, check out Saket Jain's overview, [Hyperparameter Tuning in Machine Learning](https://saketjain.github.io/posts/hyperparameter-tuning/)[^Tuning].

[^Bayesian]:

Bayesian Optimisation is a broad term that describes the strategy that lots of modern tuning frameworks use, but the specific algorithm they use varies. The most common algorithm is the Tree-Structured Parzen Estimator (TPE), which is a very fancy method that is well out of scope for this session and which you don't really need to understand yet (but if you really want to, I'd recommend this very thorough [blog post](https://anna-ssi.github.io/blog/tpe/) by Anna Hakhverdyan).
