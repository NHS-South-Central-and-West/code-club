...
This creates a `pyproject.toml`, our recipe, as well as a folder structure. We need to move our `nhsutils` package into the `src/` subfolder it has created (and delete the example folder it's created in there at the same time). Let's take a look at `pyproject.toml` now:
