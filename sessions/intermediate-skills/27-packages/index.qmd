...
What is better is using a combination of `uv` and Github, which we've both looked at in previous sessions.

### Getting a `pyproject.toml`

`uv` takes care of this for us.

```powershell
uv init --lib
```

This creates a folder structure for us. We need to move our `nhsutils` package into the `src/` subfolder it has created (and delete the example folder it's created in there at the same time). Let's take a look at `pyproject.toml` now:
...
Let's change the `name` to `nhsutils` (this needs to match the folder in `src/`). We also need to just run `uv add requests` as this is a package our ODS code lookup depends on. We should end up with:
...
This adds our package to their project. If we ever update it, it's best practice to bump the `version` number in `pyproject.toml` to avoid confusion, before checking our changes in to Github. The next time they run...
