...
Generators are a memory-efficient approach to iteration. They can be very useful when dealing with large amounts of data, and they are listed in the NHS's National Competency Framework as a requirement for intermediate Python Proficiency (SA21). In this tutorial, we are going to cover:

-   What generators are
-   When it makes sense to use generators and when not
-   Example use-cases:
...
The list evidently takes up far more space in memory than the generator. That is because the list is being created all at once (all 99,999 integers), whereas the generator is really just storing the _instructions_ for the comprehension, plus its current _state_ (i.e., which iteration it has got to).
