...
We have seen a previous example of this in the Decorators session, but there are lots of ways we can use flexible function inputs besides decorators.
...
Both are valid. A rough guide: use list comprehensions for one-off inline transformations; use `map()`/`filter()` when you have named functions you want to apply, since the code reads as a description of what is happening — "map this function over this data".
:::