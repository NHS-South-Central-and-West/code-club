...
In addition to the slideshow above there will also be some elements which learners may wish to code along with. The demonstration has been run in VS Code enabled with the following extensions:
...
**"Quarto"** is a system for publishing scientific and technical work. The name is a reference to the "Quarto" format of print publishing, where a the sheet is printed into 4 sections which can all be printed double sided. 

The Quarto system allows for multiple code languages to be combined with markdown to allow for users to format their output for readability before converting into one of a number of formats from Word documents and PDFs to Presentations and Websites (For example, this website has been built in Quarto).
...
![Find the Quarto Extension]("image/find_quarto_extension.png")
...
![viewing Quarto commands in the Command Palette]("image/find_quarto_extension.png")
