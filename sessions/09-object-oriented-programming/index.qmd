...
**Remember**: Functions precede the variable `function(my_variable)`, whereas methods are the ones that come after the variable `my_variable.method()`.
...
class SCWPlot:
    def __init__(self, x_data, y_data, title="SCW Plot", xlabel="X-axis", ylabel="Y-axis", grid=True):
        self.x_data = x_data    # x-axis values
        self.y_data = y_data    # y-axis values
        self.title = title      # the chart title (has a default value unless specified)
        self.xlabel = xlabel    # the x-axis label (has a default value unless specified)
        self.ylabel = ylabel    # the y-axis label (has a default value unless specified)
        self.grid = grid        # display a grid on the chart area (default == True)
...
In reality, you would not have all of the implementation code (i.e., the code written to create the class) in your script or notebook. This would be stored in a separate Python file, and the class would be imported into the script. In a future session, we will talk about this and create our own packages.
