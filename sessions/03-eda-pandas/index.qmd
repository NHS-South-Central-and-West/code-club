...
::: {.callout-note}
### Please note!
We assume that you've created a `uv` project with packages `jupyter`, `pandas`, `numpy` and `seaborn`, and have created a blank notebook in this project and opened it in VS Code. If you're not sure how to do this, please refer back to sessions [1 (onboarding)](../01-onboarding/index.qmd) and [2 (jupyter)](../02-jupyter_notebooks/index.qmd).
:::
...
If we want to select multiple columns, we can use double squared brackets (`[[ ]]`). This is the same process as before, but the inner brackets define a list, and the outer are the selection brackets. Lists are an example of a data (variable) type - for more detail on data types, refer to [session 6](../06-data-types/index.qmd).
